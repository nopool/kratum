...
class JobCounter {
  static maxCount = 0xffffff;
         #count   = 0;

  getCurrent() { return this.#count.toString(16); } // Unused publicly

  getNext() {
    this.#count++;
    if (this.#count > JobCounter.maxCount) this.#count = 0;
 ...
 * Emits:
 *
 *       jobCreation(blockTemplate) – when block that was previously unknown to job manager is
 *                                    surfaced; used to broadcast new job
 *         jobUpdate(blockTemplate) – when time of known block is reset; used to broadcast
 *                                    superseding job
 * shareProcessing(data, blockData) – when submitted share is processed; block data will be
 *                                    populated if share contains target hash
 */
function JobManager() {
  const _this                     = this;
  const chainCount                = config.get('coins').kadena.chainCount;
  ...
  const printValidJobs            = () => {
                                      ...
                                      for (var i = 0; i < chainCount; i++) {
                                        ...
                                        if (jobsForChain) {
                                          ...
                                          for (var height in jobsForChain) {
                                            const jobsForBlock = jobsForChain[ height ];

                                            for (var id in jobsForBlock) {
  ...
  let   heightIncreasedAt         = Date.now();
        ...
        this.addJob                 = (job, onAdded, isViaWork) => {
                                        ...
                                        initiateUpdates(job);
                                        if (onAdded) onAdded();
                                        if (isViaWork) this.emit('jobCreation', job);
                                        logger.debug(
                                          'Queued job ' + id
                                                        + ' for block '
                                                        + height
                                        ...
                                        if (jobsForChain) {
                                          ...
                                          for (var height in jobsForChain) {
                                            if (height < latestHeight) {
                                              for (var id in jobsForChain[ height ]) {
                                                invalidJobs.push(id);
                                              }
