 ...
 * updated(blockTemplate) – when time of known block is reset; used to broadcast next new job
...
const JobManager = () => {
  ...
  let   heightIncreasedAt         = Date.now();
        ...
        this.processBlockTemplate = (rpcData, nodeId, onProcessed, isViaWork) => {
                                      ...
                                      } else { // Avoiding duplicate work
                                        logger.debug(
                                          'Skipped adding stale job for block ' + height
                                                                                + ' of chain '
                                                                                + chainId
                                        );
        ...
        this.updateJob              = (job) => {
                                        ...
                                        if (!latestJob || height >= latestJob.height) {
                                          const jobsForBlock = validJobs[ chainId ][ height ];
                                          ...
                                          if (jobsForBlock) {
                                            ...
                                            logger.warn(
                                              'Skipped updating removed job ' + 'for block '
                                                                              + height
                                                                              + ' of chain '
                                                                              + chainId
                                            );
                                          ...
                                          logger.warn(
                                            'Skipped updating stale job for block ' + height
                                                                                    + ' of chain '
                                                                                    + chainId
                                          );
                                          cancelUpdates(job);
        ...
        this.getNextJob             = () => {
                                        ...
                                        if (this.getJobCount()) {
                                          ...
                                          if (!job) logger.warn('Jobs no longer available');
                                        } else {
                                          logger.warn('No available jobs');
                                        }
        ...
        this.processShare           = (
                                        ...
                                        if (job) {
                                          ...
                                          if (job.registerSubmission(nonce2)) {
                                            ...
                                            logger.info(`nonce2: ${ nonce2 }`);
                                            logger.info(`Time: ${ job.time }`);
                                            logger.info(`Header: ${ job.header }`);
        ...
        this.getJobById             = (id) => {
                                        ...
                                        if (!job) logger.warn(`No job with ID ${ id }`);
