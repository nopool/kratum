...
const coinConfigPath              = path.join(__dirname, 'coins/kadena.json');
      ...
      exports.getChainCount       = () => {
                                      let count = config.get('chainCount');
      ...
      exports.getReward           = (height) => {
                                      ...
                                      return reward && reward / exports.getChainCount();
                                    };
      ...
      exports.toSha256d           = (buffer) => {
                                    ...
                                    // Used to format PoW wallet address for use in generation
                                    // transaction’s output
      exports.addressToScript     = (address) => {
                                      const decodedAddress = base58.decode(address);

                                      if (!decodedAddress) {
                                        logger.error(`Base58-decoding failed for ${ address }`);

                                        throw new Error();
                                      }

                                      if (decodedAddress.length != 25) {
                                        logger.error(`Invalid address length for ${ address }`);
